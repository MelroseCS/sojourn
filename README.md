...
The device is assigned a Universally Unique Identifier or UUID. It is a 128-bit integer number used to identify the device installation.  It is represented as a 32 alphanumeric string in the form of `123e4567-e89b-12d3-a456-426655440000`.  The installation Identifier is immutable and remains persistent across upgrades of the app.  If the hosting application is completely removed from the device a fresh install will generate a new installationId.
...
## Sojourn Listener
Sojourn Listener is a protocol used to all applications to monitor various events generated by the SDK.  Types of event that an application can listener to are:
* Incoming Inbox Messages.
* Location Authorisation changes.
* Location events (every time a location is received from the platform).

To enable an application to receive events from sojourn you will need to conform to the SojournListener protocol and register your listener with the SDK.

```
@interface ViewController : UIViewController<SojournListener>
...
```
/**
 * Called when a location is received by Sojourn
 */
- (void)didUpdateLocation:(CLLocation* _Nullable)location movement:(double)distance
{
    NSLog(@"App got location %@", location);
}
/**
 * Called when the location authorization status updates.
 */
- (void)didUpdateAuthorizationStatus:(CLAuthorizationStatus)status
{
    NSLog(@"App got auth %d", status);
}
/**
 * Called when an incoming message is received.
 */
- (void)didReceiveInboxMessage {
    NSLog(@"App got inbox message");
}
```

```
//Register your delegate with Sojourn
[Sojourn sharedService].delegate = self;
```

## Location Shot
A location shot represents a request for location from the device.  Internally Sojourn uses an accuracy and timeout parameter to configure the maximum time to wait for to receive a location at a maximum accuracy.  This hasn't been exposed, but a helper method to trigger a location shot with default parameters (by default Sojourn will attempt to garner the best location it can).  To trigger a location shot call `getCurrentLocation`

```
[[Sojourn sharedService] getCurrentLocation:^(CLLocation * _Nullable location, NSError * _Nullable error) {
